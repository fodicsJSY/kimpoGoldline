    ...
    $.ajax({
      ...
      success: function(response){
        ...
        if (response.goToGimpoCSV && response.goToGimpoCSV.length > 0) {
          if(response.goToGimpoCSV.length > 6){
            console.log("true");
            // renderTableNotDaySum(response.goToGimpoCSV);
          }else{
            console.log("false");
            // renderTableDaySum(response.goToGimpoCSV);
          }
        } else {
          renderNoDataMessage();
        }
